#' Timeseries of monthly ET for one custom polygon
#'
#' Makes calls to the DCSE N-METRIC GetByPolygon API endpoint. Data is returned as an R data frame.
#'
#' Note that all parameters are strings. Most parameters have default values and can therefore be omitted if the defaults are acceptable.
#'
...
#'
#' @param geometry Vector of lat/long pairs defining a single polygon.
...
getNmetric_polygon <- function (geometry = '', start_date = paste0(lubridate::year(Sys.Date()), '-01-01'), end_date = as.character(Sys.Date()-60),
                               interval = 'daily', units = 'in')
{
  httr::set_config(httr::config(ssl_verifypeer=0L))  # turn off ssl_verify (for use behind firewall)

  # Format the vector of lat/longs into a single comma-separated string
  geometry <- as.character(geometry_H123E)
  geometry <- paste0(geometry, ',', collapse='')         # flatten into single string and add commas between ids
  geometry <- substr(geometry, 1, nchar(geometry)-1)     # remove comma after last id

  url <- 'https://mwdmetric2.dcse.com/METRICPortal_Test/api/QueryEt/GetByPolygon'  # URL for the endpoint

  response <- httr::GET(url, query = list(geometry   = geometry,
